...
function RealmRoles({ realmName }) {
  ...
  const { data: roles = [], isLoading: rolesLoading } = useQuery({
    ...
    queryFn: () => api.get(`/roles?realm=${realmName}`).then(res => res.data),
    enabled: !!realmName,
  });
  ...
  const { data: users = [] } = useQuery({
    ...
    queryFn: () => api.get(`/users?realm=${realmName}`).then(res => res.data),
  ...
  const { data: globalRoles = [] } = useQuery({
    ...
    queryFn: () => api.get('/roles/global/all').then(res => res.data.roles),
  ...
  const addCompositeMutation = useMutation({
    ...
    onError: (error) => {
      alert(`Failed to add composite roles: ${error.response?.data?.message || error.message}`);
    }
  });
