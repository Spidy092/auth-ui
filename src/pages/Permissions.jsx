...
function Permissions() {
  ...
  const { data: stats = {} } = useQuery({
    ...
    queryFn: () => api.get('/permissions/stats/overview').then(res => res.data)
  });
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      alert(`Failed to create permission: ${error.response?.data?.message || error.message}`);
    }
  });
  ...
  const bulkCreateMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setBulkPermissions('');
      alert(`Created ${response.data.created} permissions. ${response.data.errors} errors.`);
    },
    onError: (error) => {
      alert(`Failed to bulk create permissions: ${error.response?.data?.message || error.message}`);
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      alert(`Failed to delete permission: ${error.response?.data?.message || error.message}`);
    }
  });
  ...
  const handleBulkCreate = () => {
    if (!bulkPermissions.trim()) {
      alert('Please enter permissions');
      return;
    }
    ...
    try {
      ...
      alert('Invalid bulk format. Please use "permission:name, description" per line');
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <TabPanel value={tabValue} index={2}>
        <PermissionsTable 
          permissions={permissions.filter(p => p.is_system)} 
          onMenuClick={handleMenuClick}
          isLoading={isLoading}
        />
      </TabPanel>
...
function PermissionsTable({ permissions, onMenuClick, isLoading, hideResource = false }) {
  if (isLoading) {
